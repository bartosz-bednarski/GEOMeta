...
import { topicBoxProps } from "forum";
const anonymous = (
...
const TopicBox: React.FC<topicBoxProps> = ({
  ...
  const comments = commentsArr.length > 0 ? commentsArr : "";
  const usersIcons =
    comments !== "" && comments.length > 3
      ? comments.slice(comments.length - 3)
      : comments;
  return (
    <div className={classes["topic-container"]} id={topic_id} onClick={onClick}>
      ...
      <div className={classes["topic-container__answers-container"]}>
        ...
        >
          ...
          >
            {comments !== "" ? comments[comments.length - 1].date : dateString}
          </span>
          <span
            className={
              classes["topic-container__answers-container__last-post-box__data"]
          ...
          >
            {comments !== "" ? comments[comments.length - 1].time : timeString}
          </span>
        </div>
      </div>
    </div>
