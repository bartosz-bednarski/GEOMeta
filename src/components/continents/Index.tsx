...
import { activeContinentHandler, activeContinent, countires } from "continents";

const Continents = () => {
  const [activeContinent, setActiveContinent] = useState<activeContinent>({
    active: false,
    id: "",
    continent: "",
  });
  const [countries, setCountries] = useState<countires>([
    { country: "", img: "" },
  ]);
  const continentRef = useRef<HTMLDivElement>(null);
  const navigate = useNavigate();

  const activeContinentHandler: activeContinentHandler = async ({
    id,
    continent,
  }) => {
    setActiveContinent({
    ...
    const data: countires = await getContinent(id);
    setCountries(data);
